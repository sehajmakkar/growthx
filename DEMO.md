...
| 8 | 2:30–2:45 | **Results** (Experiments → Results) | Point at the two interval bars, then scroll to **By device** | "It reads the result honestly. The intervals overlap, so it says *not yet decisive* rather than claiming a win — and it tells you it would need 9,584 sessions an arm to settle it. But look at the breakdown: mobile went up, desktop went down. The average hides that completely." |
| 9 | 2:45–2:55 | **Learnings** (top record) | Show the new learning and its `inconclusive / low` chip | "Then it writes down what it learned — targeted by device, not a global change — and files it as inconclusive at low confidence. It doesn't get to grade its own work: the outcome comes from the statistics, not from the sentence it wrote. The next run reads this before proposing anything." |
